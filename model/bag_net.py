...
def bagnet_model_factory(version_name=BgaNetVersion.BAG_NET_9, pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    if version_name == BgaNetVersion.BAG_NET_9:
        model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[1, 1, 0, 0], **kwargs)
        return model
    elif version_name == BgaNetVersion.BAG_NET_17:
        model = BagNet(Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[1, 1, 1, 0], **kwargs)
        return model
    elif version_name == BgaNetVersion.BAG_NET_33:
